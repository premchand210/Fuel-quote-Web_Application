<?php
   ...
   if ($_SERVER['REQUEST_METHOD'] == 'POST'){ //if this is a post call
	   if($_POST['firstname']==""||strlen($_POST['firstname'])>50){ //validate name
	 
		   array_push($firstname_result,"Error: Enter your firstname in 0-50 characters");
      
       }
       if($_POST['lastname']==""||strlen($_POST['lastname'])>50){ //validate name
       
		array_push($lastname_result,"Error: Enter your last name in 0-50 characters");
 
       }
	   $emailnew=$_POST['email'];
       if (!filter_var($emailnew, FILTER_VALIDATE_EMAIL)) { 
	   array_push($email_result,"Error: Enter your valid email in 0-100 characters");}
	
       
       if($_POST['address1']==""||strlen($_POST['address1'])>100){ //validate address 
           
		   
		array_push($address1_result,"enter your address in 0-100 characters");
       ...
       if(strlen($_POST['address2'])>100){ //validate address 
		...
		array_push($address2_result,"enter your address2 in 0-100 characters");
          
	   }
	   if($_POST['city']==""||strlen($_POST['city'])>100){ //validate city
        
           array_push($city_result,"Error: enter your city name in 0-100 characters");          
	   }
       ...
       if($_POST['zip']==""||!is_numeric($_POST['zip'])||strlen($_POST['zip'])>9||strlen($_POST['zip'])<5){ //validate zip
		   ...
		   array_push($zipcode_result,"enter your zip in 5-9 numbers");
...
<?php endif ?>
				...
				<?php if(count($lastname_result)>0): ?>

<div class="error">
<?php foreach ($lastname_result as $error):?>
<p><font color="red"><?php echo $error ?></font></p>
<?php endforeach ?>
...
<?php endif ?>
                ...
                <input type="text" id="email" name="email" placeholder="Add Email address" required="required"> <br>
                <?php if(count($email_result)>0): ?>
				<div class="error">
<?php foreach ($email_result as $error):?>
<p><font color="red"><?php echo $error ?></font></p>
<?php endforeach ?>
...
<?php endif ?>
                ...
                <select type = "state" name= "state" required="state">
          ...
          <option value="WY">WY</option>
                ...
                <input type="text" id="zip" name="zip" placeholder="max 9 chracters" required="zip"> <br>
